---
id: sucursales-roles-y-permisos/activar-equipo-en-una-sucursal
title: "Activar al equipo en una sucursal"
description: "Paso a paso para elegir qué barberos y gerentes trabajan en la sucursal donde estás."
section: sucursales-roles-y-permisos
order: 150
roles: [owner, admin]
screens: [/team/activate, /team/barbers]
keywords: [activar equipo, importar equipo, activar barbero, sucursal, gerentes, comisión, renta, interruptor, guardar, equipo en otra sucursal]
related: [sucursales-roles-y-permisos/equipo-compartido-entre-sucursales, sucursales-roles-y-permisos/no-veo-a-un-barbero-en-mi-sucursal, equipo/la-lista-de-barberos]
status: review
updated: 2026-09-25
---

# Activar al equipo en una sucursal

**En resumen:** en **Equipo › Acciones › Activar equipo dentro de esta sucursal** eliges, con un interruptor por persona, quién trabaja en la sucursal que tienes elegida.

## Pasos

1. Elige arriba la sucursal donde quieres activar al equipo.
2. Ve a **Equipo**.
3. Toca **Acciones** y elige **Activar equipo dentro de esta sucursal**.
4. La pantalla dice: «Seleccione el equipo que desea importar activando el equipo en el interruptor. Haga clic en “Guardar” una vez que haya hecho sus elecciones».
5. Elige la pestaña **Barberos** o **Gerentes**. Dentro de **Barberos** verás **Comisión** y **Renta**.
6. Enciende el interruptor de cada persona que trabaja en esta sucursal.
7. Toca **Guardar**.

> [!IMPORTANT]
> Si no tocas **Guardar**, los cambios no se aplican.

## Después de activar

Al tocar **Guardar** vuelves a **Equipo**. La persona aparece en la lista de **Barberos** de esa sucursal, con su sucursal, su nivel, su correo, su teléfono y el estado **Activo**. Si trabaja en dos sucursales, repite los pasos en la otra.

Una sucursal nueva empieza con «0 Barberos». En ese caso **Equipo** muestra «Aquí podrás ver los barberos de tu equipo.» y el botón **Crear barbero comisión**.

> [!NOTE]
> Si ningún barbero tiene horario ese día, el **Calendario** dice «Nadie trabaja este día». Mira [Un barbero no aparece en el calendario](/ayuda/calendario/un-barbero-no-aparece-en-el-calendario).

## Preguntas frecuentes

**¿Apagar un interruptor borra a la persona?**
Todavía no está documentado. La persona sigue existiendo en tu empresa; lo que se cambia es dónde trabaja.

**¿Por qué no encuentro a alguien en la lista?**
Solo aparecen las personas que ya existen en tu empresa. Si no existe, créala con **Crear miembro del equipo**.
